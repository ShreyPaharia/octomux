...
The review UI tiers these: `critical` and `issue` render as **blocking** (shown expanded), while `suggestion` and `nit` render as **nits/optional** (collapsed by default). Be disciplined — reserve `critical`/`issue` for the genuinely blocking few, and use `suggestion`/`nit` for everything else.
...
- Don't ship a walkthrough without the verdict, highlights, and the scalar fields filled in.
