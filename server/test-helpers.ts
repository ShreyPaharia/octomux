...
const KV_UNAVAILABLE =
  'ctx.kv is not available until the storage task lands (W1-D, held for the bun migration)';

export interface TestHostLogEntry {
  pluginId: string;
...
export interface TestHostRegistration<T> {
  pluginId: string;
  value: T;
}

export interface TestHost {
  /** PluginContext handed to each loaded plugin, keyed by its bare id. */
  contexts: Map<string, PluginContext>;
  /** Everything registered via ctx.workflows.register(), in call order. */
  workflows: TestHostRegistration<unknown>[];
  /** Everything registered via ctx.integrations.register(), in call order. */
  integrations: TestHostRegistration<unknown>[];
  /** Everything registered via ctx.harnesses.register(), in call order. */
  harnesses: TestHostRegistration<unknown>[];
  /** Every ctx.logger call across every plugin, in call order. */
  logs: TestHostLogEntry[];
  /** apply(ctx)'s return value per plugin, in load order — await these to
   * observe an async plugin's registrations past its first `await`. */
  applyResults: Promise<void>[];
}

/**
 * In-memory `PluginContext` host for unit tests. Takes plugin OBJECTS
 * (`{ id, mod }`), never package names, so a test never touches the
 * filesystem or the real loader (`server/plugins/loader.ts`, a later task).
 *
 * Calls `mod.apply(ctx)` for each entry, in order, and captures every
 * registrar call plus every logger call into the returned `TestHost`. This is
 * a test seam, not a framework: `ctx.kv` throws on use (matches the real host
 * until the storage task lands), and `ctx.settings` is a bare per-plugin
 * in-memory map, not backed by `server/settings.ts`.
 */
export function createTestHost(opts?: { plugins?: Array<{ id: string; mod: unknown }> }): TestHost {
  ...
  for (const { id, mod } of opts?.plugins ?? []) {
    const log =
      (level: TestHostLogEntry['level']) =>
      (obj: object, msg?: string): void => {
        host.logs.push({ pluginId: id, level, obj, msg });
      };
    ...
    const ctx: PluginContext = {
      ...
      settings: {
        get: async <T = Record<string, unknown>>(): Promise<T> =>
          (settingsStore.get(id) ?? {}) as T,
        update: async (patch: Record<string, unknown>): Promise<void> => {
          settingsStore.set(id, { ...(settingsStore.get(id) ?? {}), ...patch });
        },
      },
      kv: {
        get: () => {
      ...
      workflows: {
        register: (wf) => host.workflows.push({ pluginId: id, value: wf }),
      },
      integrations: {
        register: (p) => host.integrations.push({ pluginId: id, value: p }),
      },
      harnesses: {
        register: (h) => host.harnesses.push({ pluginId: id, value: h }),
      },
    };

    host.contexts.set(id, ctx);

    const plugin = mod as { apply(ctx: PluginContext): void | Promise<void> };
    host.applyResults.push(Promise.resolve(plugin.apply(ctx)));
  }
