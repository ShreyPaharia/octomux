 ...
 * this registers `send_message`, `add_agent`, `create_schedule`, and
 * `start_loop`. Each takes structured args (no Bash string) and RPCs to the
 * main server orchestrator-action endpoint, pushing an activity update to the
 * conversation. A command with `gateCapabilityId` set (create_schedule,
 * start_loop) first blocks on a human approval card via `onGatedInvoke`; the
 * rest execute immediately with no approval gate (pre-gate behaviour).
 *
  ...
  for (const cmd of COMMANDS.filter((c) => c.mcp && !CAPABILITY_MCP_NAMES.has(c.name))) {
    ...
    server.registerTool(
      ...
      async (args: Record<string, unknown>) => {
        try {
          // A gated command blocks on the SAME human-approval card gate the
          // capability-registry tools use (create_schedule / start_loop —
          // standing automation and host shell reach must not execute on the
          // model's say-so alone). onGatedInvoke throws on reject/timeout/
          // card-failure, turned into an isError result by the catch below;
          // the action RPC only runs after approval. Kill switch and rule
          // promotion behave exactly as for create_task's gate.
          if (cmd.gateCapabilityId) {
            await onGatedInvoke({ id: cmd.gateCapabilityId, mcp: cmd.name }, cmd.tier, args);
          }
