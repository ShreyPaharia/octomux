...
import { defineStore, resetRecords } from './records.js';
...
describe('plugins/ui-registry', () => {
  ...
  it('qualifies the bare record store name and attaches the owning plugin id', () => {
  ...
  it('lists contributions from multiple plugins', () => {
    ...
    registerPluginUiPanel('reviewer-bot', { slot: 'task.badge', record: 'status', as: 'badge' });
  ...
  it('rejects a binding with no record store name', () => {
    expect(() =>
      registerPluginUiPanel('coverage-bot', {
        slot: 'task.panel',
        as: 'stat',
      } as never),
    ).toThrow(/requires a non-empty string "record"/);
  });

  it('rejects a binding with an empty record store name', () => {
    expect(() =>
      registerPluginUiPanel('coverage-bot', {
        slot: 'task.panel',
        record: '',
  ...
  it('unregisterPluginUi drops record bindings too', () => {
    registerPluginUiPanel('coverage-bot', {
      slot: 'task.panel',
      record: 'coverage',
      as: 'table',
  ...
  // Finding 4: a typo in binding.record yields a permanently empty panel with
  ...
  describe('binding.record typo diagnostic', () => {
    it('warns once when a panel binds a record store nothing defined', () => {
      const logs: string[] = [];
      setLogger(pino({ level: 'trace' }, { write: (msg: string) => logs.push(msg) }));

      registerPluginUiPanel('coverage-bot', {
        slot: 'task.panel',
        record: 'coverge', // typo of 'coverage'
        as: 'stat',
      });
    ...
    it('does not warn when the bound record store is actually defined', () => {
      const logs: string[] = [];
      setLogger(pino({ level: 'trace' }, { write: (msg: string) => logs.push(msg) }));
