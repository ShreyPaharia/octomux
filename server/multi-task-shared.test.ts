...
vi.mock('fs', (importOriginal) => {
  ...
  // `server/compute/local.ts`'s `localFiles` (what `validateRepo`/setupNone
  // now go through via `ComputeSession.files`) uses `fs.promises`, not the
  // sync surface above. Shim the async methods onto the sync mocks so
  // existsSync's per-test overrides keep working, and setup stops probing
  // the real filesystem for /repo.
